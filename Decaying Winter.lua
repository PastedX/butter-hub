...
local function RenderStepped()
    ...
    if Toggles.KillAura then
        for i = 1,#Enemies do
            if Enemies[i] ~= nil and Enemies[i]:FindFirstChild("Head") and game.Players.LocalPlayer:DistanceFromCharacter(Enemies[i].Head.Position) <= KillAuraDistance then
                local data = {
                    [1] = "gundamage";
                    [2] = Enemies[i];
                    [3] = math.huge;
                ...
                game.Workspace.ServerStuff.dealDamage:FireServer(data, nil, _G.Code1, _G.Code2)
            end
        end
        return
    end
...
local function GrabEssentials()
    ...
    repeat 
        ...
        wait(0.3) 
    until FunnyPlace["seedk"] ~= nil and FunnyPlace["keylist"] ~= nil and TempEnv["afflictstatus"] ~= nil and TempEnv["drop_slide"] ~= nil
    wait(1)
    if FunnyPlace["seedk"] ~= nil and FunnyPlace["keylist"] ~= nil then
         _G.Code1 = FunnyPlace.seedk[FunnyPlace.keylist[workspace.ServerStuff.ask:InvokeServer("mainHandler", true)]]
    end
...
local function EspHandle(v)
    ...
    if v.Name == "JointGrip" and v:FindFirstAncestor("WeaponDrops") then
        if WeaponStats == nil and game.Workspace:FindFirstChild("ServerStats") and game.Workspace.ServerStats:FindFirstChild("Statistics") and game.Workspace.ServerStats.Statistics:FindFirstChild("W_STATISTICS") then
            WeaponStats = require(game.Workspace.ServerStats.Statistics["W_STATISTICS"])
        end
...
local function SetupGUI()
    ...
    if game.Players.LocalPlayer.PlayerGui.mainHUD.TabMenu.bg:FindFirstChild("Butter/GOOD") then
        return
    end
    ...
    GoodwillTab.Name = "Butter/GOODtab"
    ...
    Watermark.Image = "rbxassetid://7924662383"
    ...
    GoodwillButton.MouseButton1Down:Connect(function()
        ...
        for i,v in pairs(game.Players.LocalPlayer.PlayerGui.mainHUD.TabMenu.bg:GetChildren()) do
            if string.sub(string.lower(v.Name), #v.Name - 2, -1) == "tab" and v.Name ~= "Butter/GOODtab" and v.Name ~= "objectivetab" then
                 v.Visible = false
            elseif v.Parent:FindFirstChild(v.Name.."tab") and v.Name ~= "Butter/GOOD" then
...
namecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if Method == "FireServer" then
        ...
        if Toggles.SilentAim and tostring(self) == "applyGore" and string.find(tostring(Args[1]), "fire") then
...
newindex = hookmetamethod(game, "__newindex", function(self, index, value)
    if Toggles.AntiRecoil then
        if index == "CFrame" and tostring(self) == "Camera" and self.CameraType ~= Enum.CameraType.Scriptable and tostring(getcallingscript()) == "mainHandler" then
            return
        end
    end
...
local function CommandHandler(msg, speaker)
    ...
    if IsInTable({"light","small","short","medium","heavy","long","shells","scrap","lit","mre","soda","beans","water","cola","bottle"}, Args.Command) then
        ...
        if Amount > 40 and Args.Command ~= "scrap" and speaker ~= game.Players.LocalPlayer then
            Amount = 40
        elseif Amount > 900 and Args.Command ~= "scrap" and speaker == game.Players.LocalPlayer then
            Amount = 900
        end
            ...
            for i,v in pairs(TargetTable) do
                if typeof(v) == "table" and v["cooldown"] ~= nil then
                    for x,y in pairs(v) do
                        ...
                        if x == "perk_mincd" or x == "vulka_ammo_usage" or string.find(tostring(x),"overheat") then
    ...
    if IsInTable({"infection","stopinfection","haltinfection","virus","blockvirus","stopvirus","haltvirus","uninfection","stopinfection","virusblock","unhaltinfection","unvirus","unstopvirus","unhaltvirus","unblockvirus","unvirusblock"}, Args.Command) then
        local Toggle = true -- last second to default to true, rather than a toggle
        if Args[1] == "true" or Args[1] == "on" then
            Toggle = true
        elseif Args[1] == "false" or Args[1] == "off" or string.sub(Args.Command, 1,2) == "un" then
    ...
    if IsInTable({"oneshot","instantkill","onetap","infdamage","unoneshot","uninstantkill","unonetap","uninfdamage"}, Args.Command) then
        local Toggle = true -- last second to default to true, rather than a toggle
        if Args[1] == "true" or Args[1] == "on" then
            Toggle = true
        elseif Args[1] == "false" or Args[1] == "off" or string.sub(Args.Command, 1,2) == "un" then
    ...
    if IsInTable({"killenemies","killall","removeenemies","killothers","enemykill","clearenemies","clearhazards","enemieskill","unkillenemies","unkillall","unremoveenemies","unkillothers","unenemykill","unclearenemies","unclearhazards","unenemieskill"}, Args.Command) then
        ...
        for i = 1,#Enemies do
            if Enemies[i] ~= nil and Enemies[i]:FindFirstChild("Head") and game.Players.LocalPlayer:DistanceFromCharacter(Enemies[i].Head.Position) <= Amount then
                local data = {
                    [1] = "gundamage";
                    [2] = Enemies[i];
                    [3] = math.huge;
...
mouse.KeyDown:Connect(function(key)
    ...
    if key == "m" and _G.Code1 ~= nil and _G.Code2 ~= nil then
        for i,v in pairs(EffectsTable) do
            if not string.find(i, "Virus") then
                EffectsTable[i].effects.currentduration = 0
            end
...
if _G.Code1 ~= nil and _G.Code2 ~= nil then
    game.StarterGui:SetCore("SendNotification", {
        Title = 'SUCCESSFULL N Key: Heal and remove effects';
        Text = "SUCCESSFULLY INJECTED, F9 FOR CMDS                    ( MADE BY Applebox And AIDEZ)";
        Icon = "rbxassetid://7924662383";
        Duration = 7;
    })
print([[
...
+ Key: Increase super run speed
    ...
    game.StarterGui:SetCore("SendNotification", {
        Title = 'TO Spawn items';
        Text = "type :Spawn (item) example :Spawn ModdedRifle";
        Icon = "rbxassetid://7924662383";
        Duration = 40;
    })
else
